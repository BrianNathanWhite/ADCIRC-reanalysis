...
for(station in df_meta_region3$stationname[1:26]) {
  
  ADC_POST_Detided_region3 %>%
  select(TIME, station) %>%
  filter('1979-01-01 01:00:00' <= TIME, TIME <= '1980-01-01 23:00:00') %>%
...
# compute kernal density estimate of each stations block maxima timeseries
density_max <- function(timeseries) {
for(i in 1:length(timeseries)) {
  
  density_max <- timeseries[[i]] %>%
                    ggplot(aes(x = max)) +
                    geom_density(col = 'blue') +
                    labs(title = labels(timeseries)[i])
  
  print(density_max)
}
}

timeseries_max <- function(timeseries) {
for(i in 1:length(timeseries)) {
  
  timeseries_max <- timeseries[[i]] %>%
                    ggplot(aes(x = as.numeric(year), y = max)) +
                    geom_line(col = 'light blue') +
                    labs(x = 'year', title = labels(timeseries)[i])
  
  print(timeseries_max)
}
}


density_max(ADC_POST_Detided_max)
density_max(NOAA_OBS_Detided_max)
timeseries_max(ADC_POST_Detided_max)
timeseries_max(NOAA_OBS_Detided_max)
```
